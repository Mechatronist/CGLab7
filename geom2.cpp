...
vector<lineSegment2D> extendedLines(vector<point2D> polygonPoints, point2D guardPoint) { //Returns polygon visible to guard.

	vector<lineSegment2D> extendedLines;
	...
	for (int i = 0; i < polygonPoints.size(); i++) {
		guardVertexEdge = extendLine(polygonPoints[i], guardPoint);
		extendedLines.push_back(guardVertexEdge);
...
bool polygonOrderSorter(const point2D& pointOne, const point2D& pointTwo) {

  if (fabs(pointOne.guardAngle - pointTwo.guardAngle) < epsilon) { // i.e. they are the same
  	if (pointOne.left) { //left
  		return pointOne.distanceFromGuard > pointTwo.distanceFromGuard; //Biggest to smallest
  	...
  	return pointOne.guardAngle < pointTwo.guardAngle;
  }
...
vector<point2D> reformPolygon(vector<point2D> polygonPoints) {

	sort(polygonPoints.begin(), polygonPoints.end(), radialSort);

	point2D prototypePoint, comparePoint;

	vector<point2D> similarAnglePoints, reformedPolygon;

	while (polygonPoints.size() != 0) {
		prototypePoint = polygonPoints[0];
		similarAnglePoints.push_back(prototypePoint);
		polygonPoints.erase(polygonPoints.begin());

		while ((polygonPoints.size() != 0) && ((fabs(prototypePoint.guardAngle - polygonPoints[0].guardAngle) < 0.05))) {
			similarAnglePoints.push_back(polygonPoints[0]);
			polygonPoints.erase(polygonPoints.begin());
		}

		if (similarAnglePoints.size() == 1) {
			reformedPolygon.push_back(prototypePoint);
		}

		else {

			for (int i = 0; i < similarAnglePoints.size(); i++) {

				if (similarAnglePoints[i].intersectionType == 0) {
					comparePoint = similarAnglePoints[i];
				}

			}

			if (prototypePoint.left) { //Furthest point first
				sort(similarAnglePoints.begin(), similarAnglePoints.end(), pointSorterNegativeDistance);
			}

			else { //Closest point first
				sort(similarAnglePoints.begin(), similarAnglePoints.end(), pointSorterDistance);
			}

			for (int j = 0; j < similarAnglePoints.size(); j++) {
				reformedPolygon.push_back(similarAnglePoints[j]);
			}

		}

		similarAnglePoints.clear();
...
// vector<point2D> reformPolygon2(vector<point2D> visiblePolygonPoints, vector<lineSegment2D> polygonEdges) {

// 	//If on same edge, sort radially
...
// 			// if (onSegment(polygonEdges[i].p1, visiblePolygonPoints[j], polygonEdges[i].p2)) {
// 			// 	edgePoints.push_back(visiblePolygonPoints[j]);
// 			// }

// 			double collinearity = collinear2(polygonEdges[i].p1, polygonEdges[i].p2, visiblePolygonPoints[j]);

// 			cout << "COLLINEAR: " << collinearity << endl;

// 			// if (collinear(polygonEdges[i].p1, polygonEdges[i].p2, visiblePolygonPoints[j])) {
// 			// 	edgePoints.push_back(visiblePolygonPoints[j]);
// 			// }
...
// 		else { //More than one point, sort radially.

// 			sort(edgePoints.begin(), edgePoints.end(), radialSort); //Smallest to largest angle wrt guard point.
...
vector<point2D> visiblePolygon(vector<point2D> polygonPoints, point2D guardPoint) { //Returns polygon visible to guard.
	...
	for (int i = 0; i < polygonPoints.size(); i++) { //Need to check if point is on a vertex. If so, just add it.
		...
		for (int k = 0; k < intersectionEvents.size(); k++) {
			...
			intersectionEvents[k].point.distanceFromGuard = intersectionEvents[k].distanceFromGuard;
			intersectionEvents[k].point.guardAngle = computedAngle(guardPoint, intersectionEvents[k].point);
			intersectionEvents[k].point.intersectionType = intersectionEvents[k].intersectionType;
			intersectionEvents[k].point.left = left(guardPoint, intersectionEvents[k].point, polygonPoints[prevPointIndex]);
	...
	visiblePolygon = reformPolygon(visiblePolygon);
